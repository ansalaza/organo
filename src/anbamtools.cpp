...
void get_breakpoints2(const int& start,  const int& end, bam1_t* alignment, abg_generate_msg& msg, std::unique_ptr<std::pair<int, int>>& ptr)
{
	//init variables
...
void abg_generate(bam1_t* read,	int rstart, int rend, abg_generate_msg& msg, std::string& seq)
{
	...
	get_breakpoints2(rstart, rend, read, msg, query_ptr);
	if(msg.successful){
